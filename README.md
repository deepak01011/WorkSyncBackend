...
- Config User Secret:
  + Find `<user_secrets_id>` at `WorkSync.Services.Api.csproj` > `UserSecretsId` (Free to change to any GUID/UUID)
...
{
  "ConnectionStrings": {
    "DefaultConnection": "Server=(localdb)\\MSSQLLocalDB;Database=WorkSync;Trusted_Connection=True;MultipleActiveResultSets=true"
  }
}
```
...
{
  "ConnectionStrings": {
    "DefaultConnection": "Data Source=<ip_address>,1433;Initial Catalog=aspnetcore-WorkSync;User ID=SA;pwd=<YourNewStrong@Passw0rd>;Integrated Security=False;ConnectRetryCount=0;MultipleActiveResultSets=True"
  }
}
```
...
```bash
dotnet build Src/WorkSync.Services.Api/WorkSync.Services.Api.csproj
dotnet run --project Src/WorkSync.Services.Api/WorkSync.Services.Api.csproj --launch-profile Dev
dotnet watch --project Src/WorkSync.Services.Api/WorkSync.Services.Api.csproj run
```
